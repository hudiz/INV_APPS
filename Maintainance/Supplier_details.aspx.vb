...
Imports System.IO
Partial Class Supplier_Supplier_details
    ...
    Public Sub load_customer_grid()
        Dim qry As String = "SELECT [cust_sup_auto_id], [cust_sup_id], [cust_sup_name], [cust_sup_ph], [cust_sup_ph1], [cust_sup_email] FROM [cust_supp_dtls] where cust_sup_sts='S'"
        Dim dt As DataTable = ConfigData.getDataToDatatable(qry)
        gv_cust.DataSource = dt
        gv_cust.DataBind()
        session_datatable_store("SUPPDTLS_cust_supp_dtls", dt)
        btnExport.Visible = dt.Rows.Count
    End Sub
    Private Sub gv_cust_Init(sender As Object, e As EventArgs) Handles gv_cust.Init
        gv_cust.DataSource = session_datatable_fill("SUPPDTLS_cust_supp_dtls")
        gv_cust.DataBind()
    End Sub
    ...
    Protected Sub OnSelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        Dim qry As String = "SELECT [cust_sup_auto_id], [cust_sup_id], [cust_sup_name], [cust_sup_addr], " & _
            " [cust_sup_ph], [cust_sup_ph1], [cust_sup_email] FROM [cust_supp_dtls]" & _
            " where cust_sup_auto_id ='" & gid.Trim & "' and cust_sup_id ='" & cid.Trim & "' and cust_sup_sts='S'"
        Dim dt As DataTable = ConfigData.getDataToDatatable(qry)
    ...
    Protected Sub CreateUserButton_Click(sender As Object, e As System.EventArgs) Handles CreateUserButton.Click
        ...
        Dim custsts As String = "S"
        custid.Text = Supp_next_ID_values()
    ...
    Protected Sub DeleteUserButton_Click(sender As Object, e As System.EventArgs) Handles DeleteUserButton.Click
        ...
        If gid <> "" Then
            Dim upby As String = User.Identity.Name.ToString.Trim()
            Dim upon As String = Date.Now.ToString("MM/dd/yyyy")
            'Dim qry As String = "delete from cust_supp_dtls where cust_sup_id='" & custid.Text.Trim & "' and cust_sup_auto_id='" & gid & "' and cust_sup_sts='S'"
            'Dim inssts As Integer = ConfigData.postDataToTable(qry)
            Dim custsts As String = "S"
    ...
    Protected Sub OnDataBound(sender As Object, e As EventArgs)
        ...
        End If
    End Sub
    Public Function Supp_next_ID_values() As String
        Dim qry As String = " select max(cust_sup_id) from cust_supp_dtls   where cust_sup_id like '" & "SUP" & "%' and cust_sup_sts='S'"
        Dim dt As DataTable = ConfigData.getDataToDatatable(qry)
        If dt.Rows.Count > 0 Then
            If Not IsDBNull(dt.Rows(0)(0)) Then
                If dt.Rows(0)(0).ToString.Length > 8 Then
                    Dim srl As Int16 = dt.Rows(0)(0).ToString.Substring(8)
                    Return "SUP" & (srl + 1).ToString("0000000")
                End If
            End If
        End If
        Return "SUP" & "0000001"
    ...
    Protected Sub ExportToExcel(sender As Object, e As EventArgs)
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Supplier_Detail.xls")
        Response.Charset = ""
